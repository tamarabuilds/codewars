...
function tribonacci(signature, n){
    ...
    // // learned solution. concise

    const result = signature.slice(0,n)
    while( result.length < n) {
        result[result.length] = result.slice(-3).reduce( (acc, c) => acc + c, 0)
    }
    return result

    // learned solution. clever
}
...
console.log(tribonacci([1,1,1], 3))  // 1